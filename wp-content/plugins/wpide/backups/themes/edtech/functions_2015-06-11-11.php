<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19f5919f0e3d"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-06-11-11.php") )  ) ){
...
?>
    <script type="text/javascript">
        ...
        jQuery(document).ready(function() {
            a=jQuery('.search').abcd();
            alert(a);
            
            function abcd(){
             bcd=jQuery('.search').suggest(se_ajax_url + '?action=se_lookup', {
                onSelect: function() {
                   // jQuery(this).focus();
                   
                }
            });
            removedSpanString = removeElements(bcd, "span");
            return removedSpanString;
            }
        });
...
function se_lookup() {
    ...
    foreach ($wpdb->get_results($query) as $row) {
        ...
        $id = $row->ID;
        echo  "<a href='".get_permalink($id)."'>".$post_title."</a>\n";
