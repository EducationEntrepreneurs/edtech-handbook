<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19f1e9d237e7"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/search.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/search_2015-06-09-09.php") )  ) ){
...
<?php
    ...
    <?php } else{ ?>
        ...
        <?php
            ...
            if($featured_one==""){
                if($featured_two==""){
                    $featured_one = search_featured("Explore");
                    $featured_two = search_featured("Explore");
                }
                else{
                    $cat_name_one = $cat_name_two;
                    $featured_one = search_featured($cat_name_one);
                }
            }else if($featured_two==""){
                $cat_name_two=$cat_name_one;
                $featured_two = search_featured($cat_name_two);
            }
        ?>
