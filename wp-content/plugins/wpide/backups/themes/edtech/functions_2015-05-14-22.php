<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "b5b36394a2b09f8ef6155ad17086bee7198e33828b"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-05-14-22.php") )  ) ){
...
function cat_sort(){
    ...
    if($select == 'recent')
    {
       $response = cat_sort_recent($cat_name,$page,$select);
    }
    else if($select == 'useful')
    {
        $response = cat_sort_useful($cat_name,$page);   
    }
    else
    {
        $response = cat_sort_featured($cat_name,$page,$select);   
    }
    echo $response;
    exit();
}
...
function cat_sort_useful($catname,$paged){
                $args = array(
	                'posts_per_page' => 4,
	                'paged' => $paged,
	                'offset' => $offset,
