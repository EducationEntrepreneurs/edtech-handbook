<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19086a843a79"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-06-09-12.php") )  ) ){
...
foreach($link_array as $link){
        ...
        echo similar_text($section_links,$link);
        exit;
    if($count==0){
        $flag=0;
     }
     
    else if($count==strlen($section_links)){
        delete_post_meta($post_id, 'jump_to_section_link',$link);
        $flag=1;
     }
    else
    {
  ...
  $link_arrays= get_post_meta($post_id,'jump_to_section_link');
if($flag==1){
   foreach($link_arrays as $link)
