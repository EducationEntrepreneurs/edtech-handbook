<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f192d64590745"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-06-17-06.php") )  ) ){
...
wp_enqueue_script( 'auto-suggest', get_template_directory_uri() . '/js/auto-suggest.js' );
...
function search_result($text,$s){
        ...
        foreach ($positions as $key => &$value) {
            if($value>=($positions[$key-1]+30)){
                $length = ($value - 20) - $j;
                if($key==($max)){
                    $text = substr_replace($text,'...',$j);
                }else{
...
function format($img,$current_position,$post_time){
    ...
    return  '<div class="sorted_res">'.$imagepath.'
                            ...
                            <div class="post_title"><h3><a href="'.get_post_permalink().'">'.$title.'</a></h3></div>
                            <div class="author_info">
                                <div class="author_dp">
                                     <a href="../../expert?id='.get_the_author_meta( 'ID' ).'">'.get_avatar( get_the_author_meta( 'ID' ), 45 )
                                .'</a></div>
                                <div class="author">                                
                                        <p><a href="../../expert?id='.get_the_author_meta( 'ID' ).'">'.get_the_author().'</a></p>
...
?>
    <script type="text/javascript">
        ...
        jQuery(document).ready(function() {
            jQuery('.search').autosuggest(se_ajax_url + '?action=se_lookup', {
                onSelect: function() {
                    window.location.href = jQuery(".ac_results li a:focus").attr("href");
                }
            });
        });
    </script>
