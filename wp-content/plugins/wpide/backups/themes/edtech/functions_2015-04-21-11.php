<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "b5b36394a2b09f8ef6155ad17086bee7bbe263e7e9"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-21-11.php") )  ) ){
...
function vote_counts()
{
   ...
   add_meta_box( 'vote_counts_yes', 'Vote Counts Yes', 'vote_counts_fnc_yes', 'post', 'normal', 'high' );
   add_meta_box( 'vote_counts_no', 'Vote Counts No', 'vote_counts_fnc_no', 'post', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'vote_counts', 10, 2 );
function vote_counts_fnc_yes(){
    echo get_post_meta($post_id,'vote_counts_yes');
}
function vote_counts_fnc_no(){
    echo "Hi No";
}
...
function vote_count(){
    ...
    update_post_meta($postId, 'vote_count_yes', '1');    
    update_post_meta($postId, 'vote_count_no', '11');    
    $ip_array=array();
