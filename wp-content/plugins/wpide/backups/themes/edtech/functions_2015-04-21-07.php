<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "b5b36394a2b09f8ef6155ad17086bee7bbe263e7e9"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-21-07.php") )  ) ){
...
function vote_update($result,$postId,$ipAddress){
    global $wpdb;
    //$wpdb->query('insert into '.$wpdb->prefix.'vote_log values ('.$postId.', '.$ipAddress.')');
    //echo($result);
//    array_push($result,$ipAddress);
    foreach($result as $test){
       $test->ip_address =$test->ip_address.';'.$ipAddress;
    }
    print_r($result);
    //$ip = array_push($result->ip_address,$ipAddress);
    //$wpdb->query("UPDATE wp_vote_log SET ip_address=".$ip." WHERE post_id=".$postId);
    $response = "Thank you for your feedback!";
    return $response;
}
