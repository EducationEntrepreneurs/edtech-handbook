<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "97555ad40172e2ae2acf6e24b6801088e95238deac"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/user.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/user_2015-04-09-05.php") )  ) ){
...
?>
    <a href="http://dev.anzum.in/edtech/experts-2/" class="all_experts">All Experts</a>
    <div class="user_container">
        <div class="wrapper_left">
            ...
            <div class="user_resources">
                <?php   $category =array(6,7);
                        $counter=1;
                        //foreach($category as $cat){
                        query_posts(array('author'=>$user_id,'category_name' => 'Explore','posts_per_page' => 3));
                        
                        if(have_posts()) :?><h3>Edtech Handbook resources by <?php echo $user->first_name; ?></h3><?php while(have_posts()) : the_post(); 
                        ?>
                        <p> <?php if(is_category('Explore')) { ?><i class="fa fa-lightbulb-o"></i><?php } 
                            elseif(is_category('Validate')) { ?><i class="fa fa-comment"></i><?php } 
                            elseif(is_category('Build')) { ?><i class="fa fa-user"></i><?php }
                            else { ?><i class="fa fa-leaf"></i><?php } ?>
                            <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                        </p>
                <?php   endwhile; endif; wp_reset_query();?>
                <?php query_posts(array('author'=>$user_id,'category_name' => 'Validate','posts_per_page' => 3));
                        if(have_posts()) :?><h3>Edtech Handbook resources by <?php echo $user->first_name; ?></h3><?php while(have_posts()) : the_post(); 
                        ?>
                        <p> <?php if(is_category('Explore')) { ?><i class="fa fa-lightbulb-o"></i><?php } 
                            elseif(is_category('Validate')) { ?><i class="fa fa-comment"></i><?php } 
