<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19a42ad1fc83"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/search.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/search_2015-06-10-07.php") )  ) ){
...
<?php
    ...
    foreach ($wpdb->get_results($query) as $row) {
        ...
        $flag++;
        $search_items = '<div class="'.$category_name[0]->name.'"><h5 class="post_category">'.$category_name[0]->name.'</h5><h3>'.$post_title.'</h3>'.$result.'</div>'; 
    ...
    if($flag>0){ ?>
        ...
        <div class="wrapper_left">
            <div class="search_results"><?php foreach($posts as $post){ echo $post; }  ?></div>
            <div style="clear:both"></div>
        <?php
            $link = './?s='.$search.'&page=%d';
            $prev_link = './?s='.$search;
            $next_page_link = './?s='.$search.'&page='.$i;
            $prev_page_link = './?s='.$search.'&page='.$j;
            ...
            if( $totalPages != 0 ) 
            {
                ...
                {   
                    for($i=$page;$i<=($totalPages-$page);$i++){ 
                        $page_links.='<a class="page-numbers" href="'.$links.'">'. ($i+1).'</a>&nbsp;';
                    }
                    $pagerContainer .= sprintf('<span class="page-numbers current">'.$page.'</span>&nbsp;'.$page_links ); 
                }
                else if($page==2){
                    for($i=$page;$i<=($totalPages-$page);$i++){
                        $next_pages.='<a class="page-numbers" href="'.$next_page_link.'">'. ($i+1).'</a>&nbsp;';
                    }
                    for($j=1;$j<=($page-$j);$j++){
                        $prev_pages.='<a class="page-numbers" href="'.$prev_page_link.'">'.$j.'</a>&nbsp;';
                    }
                ...
                { 
                    $pagerContainer .= sprintf( '<a class="prev page-numbers" href="'.$link.'">« Previous</a>&nbsp;<span class="page-numbers current">'.$page.'</span>&nbsp;', $page - 1 ); 
                }
                if( $page == $totalPages ) 
                { 
                    $pagerContainer .= ''; 
