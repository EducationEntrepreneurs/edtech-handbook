<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f1906b34389e9"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-23-12.php") )  ) ){
...
function vote_meta($cat_name){
   /* $check = get_post_meta($postID,'vote_count_yes');
    if(!empty($check)){
        foreach($check as $featured){
                $featured_post=$featured;
        }
    ...
    $vote = array();
        foreach ( $posts_array as $post ){
	        $id = $post->ID;
            $vote[] = get_post_meta($id,'vote_count_yes');
        }
        print_r($vote);
}
