<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "8e90b7f77a28aa29679f586f78763cab26ecc6a29b"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-03-24-18.php") )  ) ){
...
function post_section( $atts ) {
    ...
    $section_links = '<a href="#'.$title.'">'.$title.'</a><br/>';
    jump_to_section($section_links);
    return '<section id="'.$title.'">'.$section_content.'</section>';
}
add_shortcode( 'section', 'post_section' );
function jump_to_section($section_links){
    
    
}
function jump_to_section_link(){
    add_meta_box( 'post_link', 'jump_to_section_link', 'post', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'jump_to_section_link' );
