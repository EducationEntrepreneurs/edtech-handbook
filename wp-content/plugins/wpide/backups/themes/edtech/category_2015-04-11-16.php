<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19f999607a35"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/category.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/category_2015-04-11-16.php") )  ) ){
...
?>
    ...
    <div class="wrapper_left">
        ...
        <div class="sort">
            <?php while ( have_posts() ) : the_post(); ?>
                <?php 
                    ...
                    $unix_post_time_ago=ceil(abs($time-$unix_post_time)/60/60);
                    if($unix_post_time_ago>24){
                        $post_final_time=ceil($unix_post_time_ago/24);
                        $post_time='added '.$post_final_time.' day ago';
                    }
                    else
                    {   
                        $post_time='added '.$unix_post_time_ago.' hour ago';
                    }
                ...
                ?>
                        <div class="sorted_res">
                            ...
                            <div class="post_time"><?php echo 'added '.$unix_post_time_ago.' hours ago'; ?></div>
        ...
        <?php 
            ...
            ?>   
            <?php  while ( have_posts() ) : the_post(); ?>
                <?php  $post_gmt_time =$post->post_date_gmt;
                    $unix_post_time=strtotime("$post_gmt_time");
                    $unix_post_time_ago=ceil(abs($time-$unix_post_time)/60/60);
                    if($unix_post_time_ago>24){
                        $post_final_time=ceil($unix_post_time_ago/24);
        ...
        <div class="experts_info">
            ...
            <?php $authorID=edtech_author($category_name); 
                  foreach($authorID as $author){
                     ...
                     ?><div class="author_info">
                        <?php   echo $avatar;
                        ?><a href="../../expert0?id=<?php echo $author; ?>"><?php echo $author_name; ?></a>
                        </div><?php
                 }?>
        </div>
        <div class="suggest">
