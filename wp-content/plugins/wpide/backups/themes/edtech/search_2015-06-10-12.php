<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f1974b699a825"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/search.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/search_2015-06-10-12.php") )  ) ){
...
<?php
        ...
        <?php
            ...
            if( $totalPages != 0 ) 
            {
                ...
                /*/if($counter == 2){
                    ...
                    $pages_link = './?s='.$search.'&page=';
                }elseif($counter < 2 || $counter > ($totalPages -2)){
                    $pagerContainer .= ''; 
                }*/
                if( $page == 1 ) 
                {    $counter=1;
                    for($i=$page;$i<$totalPages;$i++){
                        if($counter == 2){
                            ...
                            $next_pages.=$pages_link.'&nbsp;';
                            $pages_link = './?s='.$search.'&page=';
                        }elseif($counter < 2 || $counter > ($totalPages - ($page+2))){   
                ...
                { 
                    ...
                    for($i=$page;$i<$totalPages;$i++){
                        if($counter == $page){
                            ...
                            $next_pages.=$pages_link.'&nbsp;';
                            $pages_link = './?s='.$search.'&page=';
                        }elseif($counter < $page || $counter > ($totalPages-($page+2))){
                            $pages_link=$pages_link.($i+1);
                            $next_pages.='<a class="page-numbers" href="'.$pages_link.'">'.($i+1).'</a>&nbsp;';
                            $pages_link = './?s='.$search.'&page=';
                        }
                    ...
                    $counter=1;
                    for($j=1;$j<$page;$j++){
                        if(($counter == ($page-3) && $counter!=1) || $counter > ($page+2) ){
                            //if($Page!=4){
                                $pages_link= '<span class="page-numbers dots">…</span>';
    ...
    <?php } else{ ?>
        <div class="search_desc">
            <p><?php echo "Oops, we couldn’t ?nd any results for "."\"". $search."\"";  ?></p>
        </div>
        <div style="clear:both"></div>
        <div class="wrapper_left">
