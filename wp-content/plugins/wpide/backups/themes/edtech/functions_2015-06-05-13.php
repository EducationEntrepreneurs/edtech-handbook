<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f190646225c3c"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-06-05-13.php") )  ) ){
...
function search_result($text,$s){
        ...
        foreach($position as $key=>$values){
            if($key>0){
                $length=$position[$key+1]-$position[$key];
            }
            else{
                $length=$values;
            }
