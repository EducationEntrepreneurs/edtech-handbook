<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "b5b36394a2b09f8ef6155ad17086bee72cb84ddb67"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-20-18.php") )  ) ){
...
function register_ajaxSort_script() {
    wp_register_script(
      'ajaxSort',
       get_stylesheet_directory_uri() . '/js/vote.js',
       array('jquery')
    );
...
function vote_count(){
    ...
    $sql = "insert into ".$wpdb->prefix. "vote_log values(".$postId.", '100.100.100.100')";
   $wpdb->query($sql);
$response = json_encode( array( 'success' => true ) );
exit();
}
add_action( 'wp_ajax_vote_count', 'vote_count' ); // ajax for logged in users
add_action( 'wp_ajax_nopriv_vote_count', 'vote_count' ); // ajax for