<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19e88604f605"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-05-14-11.php") )  ) ){
...
function all_posts($postid_array,$posttype,$page,$category_name){
...
function cat_sort(){
    ...
    {
        $response = cat_sort_featured($cat_name,$page);   
    }
    echo $response;
    exit();
}
...
function cat_sort_featured($catname,$page){
    $args = array(
	                'posts_per_page' => -1,
	                'category_name' => $catname
                );
    ...
    endwhile;
    wp_reset_query();
    $posts=all_posts($variable_post_ID,$post_featured,$page,$catname);
    return $posts;
}
