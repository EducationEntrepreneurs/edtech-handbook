<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19616e6262f5"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-13-05.php") )  ) ){
...
function post_time(){
    $time=time();
    $post_gmt_time =$post->post_date_gmt;
    $unix_post_time=strtotime("$post_gmt_time");
    $unix_post_time_diff=floor(abs($time-$unix_post_time)/60);
    if($unix_post_time_diff<1){
       $post_time='added few seconds ago';   
    }
    elseif($unix_post_time_diff>=1&&$unix_post_time_ago<60){
        if($unix_post_time_diff==1){
    ...
    {   
        $post_hours_time=$unix_post_time_ago/60;  
        ...
        elseif($post_hours_time>1&&$post_hours_time<24){
            ...
            $post_day_time=$post_hours_time/24;
            if($post_day_time==1){
