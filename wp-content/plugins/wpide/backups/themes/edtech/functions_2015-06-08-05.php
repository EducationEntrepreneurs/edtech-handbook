<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f192faefa4282"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-06-08-05.php") )  ) ){
...
function wp_trim_excerpt_do_shortcode($text,$s) {
$raw_excerpt = $text;
	if ( '' == $text ) {
...
function search_result($text,$s){
        ...
        /*$position = array();
        $last=0;
        while (($last = strpos($text,"</b>", $last))!== false) {
            $position[] = $last;
            $last = $last + strlen("</b>");
        }
        $indx=0;
        ...
        foreach($position as $values){
            /*if($indx>0){
                $length=$position[$indx]-$values+5;
                //$string=str_split($text,$length);
            }
            else{*/
        
                //$length=$values-$prev+5;
               // $string=explode("</b>".preg_match('/^[\w\-]+$/',$text)." ",$text);
                //$prev=$length;
            //}
            //$indx++;
            //$length=$values;
                ...
                //$++;
                /*$keys=$key+1;
                $length=$position[$keys]-15;
    
                if($values-$indx>=30 && $indx!=0){
                    $text = substr_replace($text,'..',$indx,$length);
                    $indx=$values+10;
                }*/
                /*else if($values>=30 && $indx==0){
                    $text = substr_replace($text,'..',$indx,$length);
                    $indx=$values+10;
                }*/
                /*else{
