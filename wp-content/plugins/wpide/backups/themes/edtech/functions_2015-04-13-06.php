<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f19616e6262f5"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/functions_2015-04-13-06.php") )  ) ){
...
function register_ajaxSort_script() {
    wp_register_script(
      'ajaxSort',
       get_stylesheet_directory_uri() . '/js/ajaxSort.js',
       array('jquery')
    );
    wp_enqueue_script('ajaxSort');
}
add_action('wp_enqueue_scripts', 'register_ajaxSort_script');
...
function post_time($postid){
    $time=time(y);
    echo $time;
    $unix_post_time =get_post_time(true,$postid);
    $unix_post_time_diff=floor(abs($time-$unix_post_time)/60);
    if($unix_post_time_diff<1){
       $post_time='added few seconds ago';   
    }
    elseif($unix_post_time_diff>=1 && $unix_post_time_diff<60){
        if($unix_post_time_diff==1){
            $post_time='added '.$unix_post_time_diff.' min ago';   
        }
        else{
            $post_time='added '.$post_final_time.' mins ago';
        }
    }
    else
    {   
        $post_hours_time=floor($unix_post_time_diff/60);
        if($post_hours_time==1){
            $post_time='added '.$post_hours_time.' hour ago';
        }
        elseif($post_hours_time>1 && $post_hours_time<24){
            $post_time='added '.$post_hours_time.' hours ago';   
        }
        else{
            $post_day_time=floor($post_hours_time/24);
            if($post_day_time==1){
                $post_time='added '.$post_day_time.' day ago';    
            }
            else{
                $post_time='added '.$post_day_time.' days ago';
            }
        }
    }
    return $post_time;
