<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5e6d9457ea217937a430bd995c5f3f192faefa4282"){
                                        if ( file_put_contents ( "/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/themes/edtech/search.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/anzumin/public_html/dev.anzum.in/edtech/wp-content/plugins/wpide/backups/themes/edtech/search_2015-06-08-06.php") )  ) ){
...
<?php
    ...
    foreach ($wpdb->get_results($query) as $row) {
        ...
        $position = array();
        $last=0;
        while (($last = strpos($post_content,"</b>", $last))!== false) {
            $position[] = $last;
            $last = $last + strlen("</b>");
        }
        $max = count($position);
